...
<?php if ( $student_details !== null ): // User didn't submit the form ?>
    ...
    <div class="row">
        ...
        <?php else: // student found ?>
            ...
            <form action="<?= site_url( 'student-attendance/process' ) . '?redirect=' . urlencode( $redirect_url ) ?>" method="post">
                    ...
                    <tbody>
                        ...
                        foreach ( $student_details as $std ): ?>
                            <tr>
                                ...
                                <td><?= $std['gender'] ?></td>
                                <td>
                                    <input type="hidden" name="student[<?= $count ?>][student_id]" value="<?= $std['student_registration_id'] ?>">
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="student[<?= $count ?>][attendance]" value="present" <?= set_radio( 'student[' . $count . '][attendance]', 'present', true ) ?>> Present
                                        </label>
                                    </div>
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="student[<?= $count ?>][attendance]" value="late"<?= set_radio( 'student[' . $count . '][attendance]', 'late' ) ?>> Late
                                        </label>
                                    </div>
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="student[<?= $count ?>][attendance]" value="absent"<?= set_radio( 'student[' . $count . '][attendance]', 'absent' ) ?>> Absent
                                        </label>
                                    </div>
                                </td>

                                <td>
                                    <img src="<?= base_url( $std['profile_pic'] ) ?>" height="80">
                                </td>
                            </tr>
                ...
                <div class="col-sm-6 col-sm-offset-3">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" name="attendance_date" value="<?= date( 'Y-m-d', now() ) ?>" placeholder="i.e. 2017-12-30">
                    </div>

                    <!-- Button -->
                    <div class="text-center">
                        <button style="width: 50%;" type="submit" name="submit" class="form contact-form-button light-form-button oswald light">Submit Attendance</button>
                    </div>
                </div>
            </form>
